...
export const ConferencePublicationRequest = () => {
    const [publications, setPublications] = useState([]);

    // Fetch pending conference publications from backend
    useEffect(() => {
        const fetchPublications = async () => {
            try {
                const response = await axios.get("http://localhost:5000/api/conference-publications/pending");
    ...
    // Function to approve a publication (changes status to "approved")
    const approvePublication = async (id) => {
        try {
            await axios.put(`http://localhost:5000/api/conference-publications/approve/${id}`);
    ...
    // Function to reject a publication (deletes it)
    const rejectPublication = async (id) => {
        try {
            await axios.delete(`http://localhost:5000/api/conference-publications/reject/${id}`);
    ...
    return (
        <div className="min-h-screen bg-gray-100 py-10 px-6">
            <div className="max-w-5xl mx-auto bg-white p-6 rounded-lg shadow-md">
                <h1 className="text-3xl font-bold text-center text-gray-800 mb-6">Conference Publication Requests</h1>
                    ...
                    <div className="space-y-6">
                        {publications.map((publication) => (
                            <div key={publication._id} className="bg-gray-50 p-5 rounded-lg shadow-sm border border-gray-200">
                                ...
                                <p className="text-gray-700"><strong>Faculty Email:</strong> {publication.facultyEmail}</p>
                                <p className="text-gray-700"><strong>Conference Name:</strong> {publication.conferenceName}</p>
...
export default ConferencePublicationRequest;
